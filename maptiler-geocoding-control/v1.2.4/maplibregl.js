...
function Yc(n) {
  ...
  return ~(d = at(n)) && (v = G[d] = Y[d](n)), {
    ...
    p(x, B) {
      ...
      let Q = d;
      d = at(x), d === Q ? ~d && G[d].p(x, B) : (v && (Bo(), ee(G[Q], 1, 1, () => {
        G[Q] = null;
  ...
  if (arguments.length === 1) {
    ...
    if (V(arguments[2], gn) && arguments[0] instanceof Object && arguments[1] instanceof Mt)
      ...
      for (var h = arguments[0], g = arguments[1], p = arguments[2], y = g.getChildBoundables(), d = 0; d < y.size(); d++) {
        var v = y.get(d);
        t.getIntersectsOp().intersects(v.getBounds(), h) && (v instanceof Mt ? t.query(h, v, p) : v instanceof Ke ? p.add(v.getItem()) : X.shouldNeverReachHere());
      }
  }
};
...
var Gu = function(n) {
  ...
  return t.prototype.createParentBoundablesFromVerticalSlices = function(i, o) {
    ...
    if (arguments.length === 1) {
      ...
      var v = arguments[0], C = arguments[1], I = arguments[2], N = new Ke(v, C), L = new bt(this.getRoot(), N, I);
...
function Wf(n, t, e, r) {
  ...
  return function(l) {
    ...
    function x(B, Q) {
      var _t = i(B, Q);
      if (g && p.push([B, Q]), L)
        y = B, d = Q, v = _t, L = !1, _t && (c.lineStart(), c.point(B, Q));
      else if (_t && N)
        c.point(B, Q);
      else {
        var Kt = [C = Math.max(Fo, Math.min(Zi, C)), I = Math.max(Fo, Math.min(Zi, I))], zt = [B = Math.max(Fo, Math.min(Zi, B)), Q = Math.max(Fo, Math.min(Zi, Q))];
...
function Hf(n, t) {
  ...
  for (var a = 0, u = n.length; a < u; ++a)
    if (c = (l = n[a]).length)
      for (var l, c, f = l[c - 1], h = f[0], g = f[1] / 2 + Ka, p = Dt(g), y = Tt(g), d = 0; d < c; ++d, h = C, p = N, y = L, f = v) {
        var v = l[d], C = v[0], I = v[1] / 2 + Ka, N = Dt(I), L = Tt(I), T = C - h, R = T >= 0 ? 1 : -1, D = R * T, b = D > ut, A = p * N;
