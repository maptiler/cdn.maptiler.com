...
var webMercProjection = require('web-merc-projection');
...
function getZoomLevelResolution(latitude, zoom, tileSize = 512) {
  return Math.cos(latitude * Math.PI / 180) * 2 * Math.PI * 6378137 / (tileSize * 2 ** zoom) * 3;
}
...
function tileCover(coordinates, zoom, tileSize) {
  ...
  for (const sample of samples) {
    ...
    const [tileX, tileY] = webMercProjection.llToTile(coordinate, zoom, tileSize);
...
function getElevation(coord, tile, tileSize, tileImage, elevationParser = defaultElevationParser) {
  ...
  let [x, y] = webMercProjection.llToTilePx(coord, tile, tileSize);
...
async function profileLineString(path, options) {
...
async function getTiles(tiles, tileRequest) {
