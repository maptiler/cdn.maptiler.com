...
import { OrthographicCamera as l, Scene as s, RawShaderMaterial as n, GLSL3 as c, Vector2 as u, WebGLRenderTarget as v, LinearFilter as h, RGBAFormat as m, Mesh as d, PlaneGeometry as p, DataTexture as x, WebGLRenderer as f } from "three";
...
const A = 6378137, I = 2 * Math.PI * A;
function D(e2, t2) {
  return E(e2, t2).map((e3) => e3 * I);
}
function z(e2, t2, o2) {
  const r2 = Math.floor(t2 * e2.width), i2 = Math.floor(o2 * e2.height), a2 = (T && P || (T = document.createElement("canvas"), P = T.getContext("webgl2")), P);
  if (null == a2)
...
function O(e2, t2, o2 = { inputName: "v", outputName: "c" }) {
  ...
  for (let e3 = 0; e3 < r2.length; e3++) {
    const a2 = r2[e3];
    0 === e3 && (i2 += `vec4 ${o2.outputName}=${w(a2.color)};
`);
    const l2 = a2.value, s2 = w(a2.color);
    if (t2 && e3 + 1 < r2.length) {
      const t3 = r2[e3 + 1].value, a3 = w(r2[e3 + 1].color), n2 = 0 === parseFloat((l2 - t3).toFixed(7)) ? "0." : `(${o2.inputName} - ${l2.toFixed(7)}) / (${(t3 - l2).toFixed(7)})`;
...
class B extends U {
  constructor(e2) {
    ...
    super(L(t2) + `vec4 c=${w(o2)};c.a*=v*${r2.toFixed(3)};`, t2);
  }
}
class W {
  constructor(e2) {
...
class Z {
  constructor(e2, t2, o2 = function() {
    ...
    this.size = new u(e2, t2), this.renderTarget = new v(e2, t2, { minFilter: h, magFilter: h, format: m }), this.camera.matrixAutoUpdate = false, this.renderer = o2, this.renderer.setPixelRatio(1), this.renderer.autoClear = false;
  ...
  setSize(e2, t2) {
    e2 === this.size.x && t2 === this.size.y || (this.size.x = e2, this.size.y = t2, this.renderTarget = new v(e2, t2, { minFilter: h, magFilter: h, format: m }), this.renderer.setRenderTarget(this.renderTarget));
  }
  setVertexShader(e2) {
...
class Q extends K {
  ...
  init(e2, t2 = null, o2 = null) {
    this.loadLowerZoomLevels = e2.loadLowerZoomLevels ?? true, this.coloringFragments = t2, this.multiChannelColoringFragment = o2, this.repaintOnPausedAnimation = e2.repaintOnPausedAnimation ?? true, this.onMoveEndListener = this.onMoveEnd.bind(this), this.onResizeListener = this.onResize.bind(this), this.onMoveListener = this.onMove.bind(this), this.tilegrid = i({ minZoom: e2.minZoom ?? 0, maxZoom: e2.maxZoom ?? 4 }), e2.bounds && (this.extent = D(e2.bounds[0], e2.bounds[1]).concat(D(e2.bounds[2], e2.bounds[3])));
    let r2 = "";
  ...
  getVisibleExtent(o2) {
    ...
    let i2 = D(r2.getWest(), r2.getSouth()).concat(D(r2.getEast(), r2.getNorth()));
  ...
  updateSlippyTiles() {
    ...
    const r2 = e2.data, i2 = t2.data, a2 = this.getMapOrThrow().getZoom(), l2 = this.getWantedTiles(a2), s2 = {}, n2 = this.tilegrid.getMinZoom(), c2 = this.tilegrid.getMaxZoom(), u2 = Math.min(c2, Math.max(n2, Math.floor(a2))), v2 = Math.pow(2, u2), h2 = 1 / v2, m2 = this.getMapOrThrow().getCenter(), d2 = this.tilegrid.getTileCoordForCoordAndZ(D(m2.lng, m2.lat), u2), p2 = a2 - u2, x2 = this.slippyTiles.length;
    ...
    for (let t3 = 0; t3 < x2; t3++) {
      ...
      for (let c3 = 0; c3 < n3; c3++) {
        ...
        if (S2 < 0 || S2 >= v2 || !l2[`${u2}/${g2}/${S2}`]) {
          m3.visible = false;
          continue;
        }
        m3.visible = true;
        const y2 = (g2 % v2 + v2) % v2;
        s2[`${u2}/${y2}/${S2}`] = true;
        const T2 = v2 - S2 - 1;
        m3.scale.x = h2, m3.scale.y = h2, m3.position.x = h2 * (g2 + 0.5), m3.position.y = h2 * (T2 + 0.5) - 1;
        let P2 = this.getTilePlacement(r2, true, u2, y2, S2), w2 = this.getTilePlacement(i2, true, u2, y2, S2);
        if (P2 && !w2 && (w2 = P2), !P2 && w2 && (P2 = w2), !P2 || !w2) {
          let t4 = e2;
          for (; !P2 && (t4 = this.getNextFrame(t4, -1), t4); )
            P2 = w2 = this.getTilePlacement(t4.data, false, u2, y2, S2);
          if (!P2 || !w2) {
            m3.visible = false;
            continue;
          }
        ...
        R2.uniforms.localSmoothing.value = this.material.uniforms.localSmoothing.value, R2.uniforms.maxSmoothingDistance.value = this.material.uniforms.maxSmoothingDistance.value, R2.uniforms.smoothingDistanceDecayFactor.value = this.material.uniforms.smoothingDistanceDecayFactor.value, R2.uniforms.timeInterpolation.value = this.material.uniforms.timeInterpolation.value, R2.uniforms.categorySmoothTransition.value = this.material.uniforms.categorySmoothTransition.value, R2.uniforms.opacity.value = this.material.uniforms.opacity.value, R2.uniforms.tex0.value = P2.tile.texture, R2.uniforms.tex0xy.value = P2.xy, R2.uniforms.tex0size.value = P2.size, R2.uniforms.tex1.value = w2.tile.texture, R2.uniforms.tex1xy.value = w2.xy, R2.uniforms.tex1size.value = w2.size, R2.uniforms.tilePixelSize.value = (_a = P2.tile.texture) == null ? void 0 : _a.image.width, R2.uniforms.time.value = o2, R2.uniforms.zoomDelta.value = p2, m3.material = R2, R2.uniforms.zoom.value = a2, f2.set(`${t3} ${c3}`, { material: R2, tileA: P2, tileB: w2, x: t3, y: c3, z: u2 });
      }
    }
  ...
  computeCurrentMixedImage({ zxy: e2 = "0/0/0", blurKernel: t2 = 0, outputSize: o2 = -1, channel: r2 = "r" }) {
    ...
    const v2 = this.bluringNodePasses[0], h2 = this.bluringNodePasses[1], m2 = this.bluringNodePasses[2];
    if (v2.setSize(c2, u2), h2.setSize(c2, u2), h2.setSize(c2, u2), v2.setFragmentShader("precision highp float;precision highp int;precision highp sampler2D;uniform sampler2D imageA;uniform sampler2D imageB;uniform float mixValue;in vec2 vUv;out vec4 fragColor;void main(){fragColor=mix(texture(imageA,vUv),texture(imageB,vUv),mixValue);}"), v2.setDefine("CHANNEL", r2), v2.setUniform("imageA", s2), v2.setUniform("imageB", n2), v2.setUniform("mixValue", l2), v2.process(), 0 === t2) {
      const e3 = v2.getPixelData();
    ...
    h2.setUniform("direction", new a.Vector2(1, 0)), h2.setUniform("kernelSize", t2), h2.setUniform("imgToBlur", v2.getOutputTexture()), h2.setFragmentShader(k), h2.process(), m2.setUniform("direction", new a.Vector2(0, 1)), m2.setUniform("kernelSize", t2), m2.setUniform("imgToBlur", h2.getOutputTexture()), m2.setFragmentShader(k), m2.process();
...
class ee extends Q {
  constructor(e2, t2, o2, r2) {
    ...
    __publicField(this, "tileGridPass", 0);
  ...
  onResize() {
    ...
    const i2 = Math.sqrt(t2 * t2 + o2 * o2) * (1 === devicePixelRatio ? 2 : 1);
    this.rttMaterial.uniforms.canvasDiagonal.value = i2, r2 = Math.min(r2, this.getRendererOrThrow().capabilities.maxTextureSize), this.accumulator.setSize(r2, r2), this.particleMaterial.uniforms.screenSize.value = r2 / this.pixelRatio, this.scene.remove(this.accumulatorDrawMesh), this.scene.add(this.accumulatorDrawMesh);
  }
  onMoveEnd() {
  }
  refresh() {
    ...
    if (!e2)
      throw new Error("The extent is null");
    const t2 = Math.max(e2[2] - e2[0], e2[3] - e2[1]) / I, o2 = [0.5 + (e2[2] + e2[0]) / 2 / I - t2 / 2, 0.5 - (e2[3] + e2[1]) / 2 / I - t2 / 2];
  ...
  prerenderInternal() {
    ...
    if (o2) {
      this.forceRender = false, this.rttMaterial.uniforms.timestep.value = this.particleSpeed * t2, this.getRendererOrThrow().setRenderTarget(this.flipFlop ? this.particleTexture1 : this.particleTexture0), this.getRendererOrThrow().copyFramebufferToTexture(new a.Vector2(0, 0), this.particleTexturePrev), this.prevRenderTime = this.lastRenderTime, this.lastRenderTime = e2;
...
let te = _te;
const oe = { maxAmount: 128, color: [255, 255, 255, 0], density: 200, size: 1, speed: 15e-4, refreshInterval: 200, fadeFactor: 0.04, opacity: 1, colorramp: te.builtin.VIRIDIS, smooth: true, fastColor: [255, 255, 255, 255], fastSpeed: 3, fastIsLarger: true };
class re extends ee {
  constructor(e2 = {}) {
  ...
  pickAt(e2, t2) {
    ...
    return { speedMetersPerSecond: a2, speedKilometersPerHour: 3.6 * a2, speedMilesPerHour: 2.23694 * a2, speedFeetPerSecond: 3.28084 * a2, speedKnots: 1.94384 * a2, directionAngle: l2, compassDirection: N(l2 + 180) };
  }
  getColorRamp() {
